...
from qbml.dynamics.predictionsim import simulation
...
def main(cfg: DictConfig):
    ...
    # Grab the predicted spectral densities.
    prediction_dataset = torch.load(cfg.predictions, weights_only=False)
    n_predictions = len(prediction_dataset)
    ...
    spd_params = []
    # Run the simulations.
    for sim in tqdm(range(n_predictions)):
        tomo, spds = simulation(
            β,
            BETA,
            HBAR,
            qubit_frequency,
